...
export class KaminoManager {
  ...
  async fullRemoveReserveFromVaultIxs(
    ...
    const investIx = await this.investSingleReserveIxs(signer, kaminoVault, reserveWithAddress, vaultReservesMap);
  ...
  async buyVaultSharesIxs(
    ...
    return this._vaultClient.buySharesIxs(user, vault, tokenAmount, vaultReservesMap, farmState, flcFarmState, payer);
  }
  ...
  /**
   * This will get the wallet type, admins number and threshold for the given wallet
   * @param wallet - the wallet to get the wallet info for
   * @returns the wallet type, admins number and threshold
   */
  static async getWalletType(wallet: Address): Promise<WalletType> {
    return KaminoManager.getWalletInfo(wallet);
  }
