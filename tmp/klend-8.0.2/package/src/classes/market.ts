...
export class KaminoMarket {
  ...
  static async loadMultiple(
    ...
    const marketReservesByMarket = withReserves
      ? await getReservesForMarkets(markets, connection, programId, recentSlotDurationMs, oracleAccounts)
...
async function initializeKaminoReserves(
  ...
  return reservesAndOracles.map(([{ address: reserveAddress, state: reserve }, oracle]) => {
    if (!oracle) {
      throw Error(
        `Could not find oracle for ${parseTokenSymbol(
          reserve.config.tokenInfo.name
    ...
    return KaminoReserve.initialize(reserveAddress, reserve, oracle, rpc, recentSlotDurationMs, cdnResourcesData);
  });
}
