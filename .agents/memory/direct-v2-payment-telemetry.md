...
This is not limited to malformed fixtures: the successful standard-v2 payment proof on the same date also recorded `hasPaymentHeader: false` on its direct authorized interaction. Treat successful payment intents as the authority for revenue and payment completion.