...
export class KaminoVaultClient {
  ...
  async createVaultFarm(
    ...
    useDevnetFarms: boolean = false,
    updatePendingFarmAdmin: boolean = true
    ...
    if (updatePendingFarmAdmin) {
      const updatePendingFarmAdminIx = await getUpdateFarmPendingAdminIx(
        this.getRpc(),
        signer,
        farm.address,
        FARMS_ADMIN_MAINNET,
      ...
      updateFarmIxs.push(updatePendingFarmAdminIx);
    }
  ...
  async createVaultFLCFarm(
    ...
    const createVaultFarm = await this.createVaultFarm(signer, vaultAddress, vaultSharesMint, useDevnetFarms, false);
    const farmsSDK = new Farms(this._rpc, this._farmsProgramId);
    const updateCooldownIx = await farmsSDK.updateFarmConfigIx(
      signer,
      createVaultFarm.farm.address,
      DEFAULT_PUBLIC_KEY,
      new FarmConfigOption.WithdrawCooldownPeriod(),
      FLC_FARM_COOLDOWN,
      undefined,
      undefined,
      true
  ...
  async withdrawEverythingAndBlockInvestReserve(
    ...
    const investIx = await this.investSingleReserveIxs(admin, vault, reserveWithAddress, vaultReservesMap, true);
  ...
  async withdrawEverythingFromAllReservesAndBlockInvest(
    ...
    const investIxs = await this.investAllReservesIxs(investPayer, vault, slot, true);
  ...
  async buySharesIxs(
    ...
    return this.buildShareEntryIxs('buy', user, vault, tokenAmount, vaultReservesMap, farmState, flcFarmState, payer);
  }
  ...
  private async buildShareExitIxs(
    ...
    if (hasAllocatedReserves) {
      ...
      if (mode === 'withdraw' && tokensToWithdraw.lte(availableTokens)) {
  ...
  private async buildReserveExitIxs({
    ...
    if (tokenLeftToWithdraw.lte(0)) {
      ...
      const reserveAllocationExecutableLiquidityToWithdraw = KaminoVaultClient.getExecutableReserveWithdrawLiquidityMap(
        reserveAllocationAvailableLiquidityToWithdraw
      );
      const reserveAllocationAvailableLiquidityToWithdrawSorted = [
        ...reserveAllocationExecutableLiquidityToWithdraw.entries(),
  ...
  async investAllReservesIxs(
    ...
    for (let index = 0; index < allReserves.length; index++) {
      ...
      const reserveAllocationLiquidityAmount = lamportsToDecimal(
        KaminoReserve.cTokensToLiquidity(currentCTokenAllocation, reserveCollExchangeRate),
        vaultState.tokenMintDecimals.toNumber()
      );
   ...
   */
  public getRefreshObligationAndReservesIxs(
    market: KaminoMarket,
    ...
    const firstRefreshObligationIx = obligation.getRefreshObligationIx();
    const refreshObligationIx = obligation.getRefreshObligationIx({
  ...
  /**
   * Computes the maximium value a given vault can invest into a given reserve based on vault allocation cap for that reserve and the reserve deposit cap
   ...
   * @returns the maximum amount that can be invested into the reserve
   */
  async getMaxInvestableFromVaultInReserve(vault: KaminoVault, reserve: KaminoReserve, slot: Slot): Promise<Decimal> {
    ...
    const allocationCapStr = targetReserveAllocation.tokenAllocationCap.toString();
    const targetReserveAllocationCap = new Decimal(allocationCapStr);
    const targetReserveExchangeRate = reserve.getEstimatedCollateralExchangeRate(slot, 0);
    const investedFromVaultInReserve = KaminoReserve.cTokensToLiquidity(
      new Decimal(targetReserveAllocation.ctokenAllocation.toString()),
      targetReserveExchangeRate
    );
    const maxInvestableUntilAllocationCap =
      allocationCapStr === U64_MAX
        ? new Decimal(U64_MAX)
        : targetReserveAllocationCap.sub(investedFromVaultInReserve).gt(0)
        ? targetReserveAllocationCap.sub(investedFromVaultInReserve)
        : new Decimal(0);
    ...
    return Decimal.min(maxInvestableUntilAllocationCap, maxInvestableInTargetReserveBasedOnReserveCap);
  }
  ...
  private static buildInstantWithdrawPlan(
    ...
    const withdrawalPenalty = Decimal.max(
      grossAmount.mul(penalties.withdrawalPenaltyBps).div(FullBPSDecimal).ceil(),
      penalties.withdrawalPenaltyLamports
    );
  ...
  private async getExpectedRedeemInKindCtokenAmount(
    ...
    const reserveCtokensOwned = postWithdrawLiquidity
      ...
      : new Decimal(this.getVaultAllocations(vaultState).get(reserveAddress)?.ctokenAllocation.toString() ?? '0');
  ...
  async loadVaultFarmStates(
    ...
    const farmStates = await FarmState.fetchMultiple(this.getConnection(), addresses, this._farmsProgramId);
    const result = new Map<Address, FarmState>();
    farmStates.forEach((state, i) => {
      if (state) {
        result.set(addresses[i], state);
      }
    });
    return result;
  }
  ...
  async loadVaultFarmState(vaultState: VaultState): Promise<FarmState | null> {
    ...
    const farmState = await FarmState.fetch(this.getConnection(), vaultState.vaultFarm, this._farmsProgramId);
  ...
  async getVaultTheoreticalAPY(
    ...
    vault.vaultAllocationStrategy.forEach((allocationStrategy) => {
      ...
      const reserveAPR = new Decimal(reserve.calculateSupplyAPR(slot, 0));
...
export class KaminoVault {
  ...
  async depositIxs(
    ...
    return this.client.depositIxs(user, this, tokenAmount, vaultReservesMap, farmState, flcFarmState, payer, memo);
  }
  ...
  async withdrawIxs(
    ...
    this.vaultReservesStateCache = vaultReservesMap;
    return this.client.withdrawIxs(user, this, shareAmount, slot, vaultReservesMap, farmState, flcFarmState, payer);
  }
...
export class KaminoVaultConfig {
  ...
  /** Minimum deposit amount in base units (lamports). Default: 1000 */
  readonly minDepositAmount: number;
  /** Minimum withdraw amount in base units (lamports). Default: 10 */
  readonly minWithdrawAmount: number;
  /** Minimum invest amount in base units. Default: 0 */
