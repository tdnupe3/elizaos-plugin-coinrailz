...
router.get('/demo-showcase', async (req: Request, res: Response) => {
  try {
    const showcase: any = {
      title: 'Coin Railz Satellite Data - x402 Payment Demo',
      description: 'Live demonstration of x402 micropayment-protected satellite data APIs',
      timestamp: new Date().toISOString(),
      steps: [],
    };

    const products = SATELLITE_DATA_PRODUCTS;
    const fireProduct = products.find(p => p.id === 'sat_fire_alerts')!;
    const weatherProduct = products.find(p => p.id === 'sat_weather_imagery')!;
    const airProduct = products.find(p => p.id === 'sat_air_quality')!;

    showcase.steps.push({
      step: 1,
      title: 'Agent discovers x402-protected endpoints',
      description: 'AI agent sends GET request, receives HTTP 402 with payment instructions',
      x402Challenge: generateX402PaymentRequired(fireProduct, req),
    });

    showcase.steps.push({
      step: 2,
      title: 'Available satellite data products',
      description: 'All 6 products with sub-$0.15 micropayment pricing',
      catalog: products.map(p => ({
        id: p.id,
        name: p.name,
        price: `$${p.priceUsd}`,
        unit: p.unit,
        source: p.dataSource === 'nasa' ? 'NASA Earthdata' : 'ESA Copernicus',
      })),
    });

    const liveDataResults: any[] = [];

    try {
      const fireData = await satelliteDataService.getFireAlerts(
        { west: -125, south: 32, east: -114, north: 42 }, 1
      );
      liveDataResults.push({
        product: 'Active Fire Alerts',
        source: 'NASA FIRMS',
        price: '$0.05',
        result: {
          fireCount: fireData.fires.length,
          sampleFires: fireData.fires.slice(0, 3),
          dataDate: fireData.dataDate,
          provenance: fireData.provenance,
        },
      });
    } catch (e: any) {
      liveDataResults.push({ product: 'Active Fire Alerts', error: e.message });
    }

    try {
      const weatherData = await satelliteDataService.getWeatherImagery(37.7749, -122.4194, 'MODIS_Terra_CorrectedReflectance_TrueColor');
      liveDataResults.push({
        product: 'Weather Satellite Imagery',
        source: 'NASA GIBS',
        price: '$0.02',
        result: weatherData,
      });
    } catch (e: any) {
      liveDataResults.push({ product: 'Weather Satellite Imagery', error: e.message });
    }

    try {
      const airData = await satelliteDataService.getAirQuality(37.7749, -122.4194);
      liveDataResults.push({
        product: 'Air Quality Index',
        source: 'OpenAQ',
        price: '$0.05',
        result: {
          measurements: (airData as any).measurements?.length || 0,
          sampleData: (airData as any).measurements?.slice(0, 2),
        },
      });
    } catch (e: any) {
      liveDataResults.push({ product: 'Air Quality Index', error: e.message });
    }

    showcase.steps.push({
      step: 3,
      title: 'Live satellite data (real NASA/ESA responses)',
      description: 'Actual data from NASA and ESA APIs - not simulated',
      liveData: liveDataResults,
    });

    showcase.steps.push({
      step: 4,
      title: 'x402 payment flow complete',
      description: 'Agent pays $0.05 USDC on Base, receives authenticated data access',
      paymentDetails: {
        protocol: 'x402 v2',
        settlement: 'USDC on Base (eip155:8453)',
        payTo: process.env.COINRAILZ_WALLET || '0xa4bBE37f9A6Ae2dc36a607B91eB148C0ae163C91',
        facilitator: getFacilitatorUrl(),
        multiChainSupport: ['Base', 'Ethereum', 'Polygon', 'Arbitrum'],
      },
    });
    ...
    showcase.summary = {
      totalProducts: products.length,
      averagePrice: '$0.075',
      dataSources: ['NASA Earthdata', 'ESA Copernicus', 'OpenAQ'],
      liveDataFetched: liveDataResults.filter(r => !r.error).length,
      errors: liveDataResults.filter(r => r.error).length,
      valueProposition: 'FREE public satellite data repackaged via x402 micropayments for AI agents',
    };
