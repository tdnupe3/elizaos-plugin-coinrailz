...
- **Stripe Connect: APPROVED (Mar 25 2026)** — Kellogg Holdings LLC is approved to create live connected accounts and charges. DO NOT build this yet — platform is in discovery phase with no marketplace volume. Build when there are real agent sellers or IoT device operators asking to get paid out. When ready: (1) the AI marketplace `releaseEscrowToAgent()` in `server/routes/aiMarketplaceRoutes.ts` already has the Phase 2 stub with `TODO: stripe.transfers.create()` — Connect completes this; (2) IoT device operators could register as connected accounts and receive USDC-equivalent payouts for data sold via x402 A2D payments; (3) Use Stripe-hosted onboarding (fastest path), direct charges, Stripe handles negative balance liability. The `agent_payouts` DB table does not yet exist — needs to be created when implementing. Use Accounts v2 API (not v1 legacy). Platform fee: 15% via `application_fee_amount` on direct charges.
