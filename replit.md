...
- **GPT IN-CHAT CREDIT PURCHASE (Dec 18, 2025)**: Added 3 new endpoints for purchasing credits directly in ChatGPT: `/api/gpt/credits/packages`, `/api/gpt/credits/create-session`, `/api/gpt/credits/status`. Clean branded URLs: `https://coinrailz.com/pay/{sessionId}` redirect to Stripe checkout. **PRODUCTION VERIFIED**: Full redirect chain tested working (coinrailz.com/pay → API redirect → Stripe checkout → 200). Uses Drizzle ORM `db` directly (not pool) so HTTP mode works. **⚠️ SCALING NOTE**: Current implementation uses in-memory `gptSessionCache` - works fine for single instance but requires Redis migration before enabling Replit autoscale or multi-instance deployment.
