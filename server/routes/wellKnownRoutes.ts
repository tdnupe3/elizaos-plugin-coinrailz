...
router.get('/.well-known/agent.json', async (req: Request, res: Response) => {
  ...
  const a2aAgentCard = {
    ...
    skills: [
      ...
      {
        id: "prediction_market_events",
        name: "Prediction Market Events (via Polymarket)",
        description: "Get trending prediction market events with volume and odds. Returns data from Polymarket, the leading prediction market platform. Use when user asks 'prediction market trends', 'trending prediction markets', 'election odds', 'event probabilities', or 'Polymarket events'.",
        inputSchema: {
          type: "object",
          properties: {
            limit: { type: "number", title: "Number of Events", description: "Max 50" },
            active: { type: "boolean", title: "Active Markets Only" },
            sortBy: { type: "string", enum: ["volume", "startDate"], title: "Sort Order" }
          }
        },
        outputSchema: {
          type: "object",
          properties: {
            events: { type: "array", title: "Prediction Market Events" },
            count: { type: "number", title: "Number of Events" }
          }
        },
        pricing: { amount: SERVICE_PRICING_USD["polymarket-events"], currency: "USD" },
        category: "prediction_markets"
      },
      {
        id: "prediction_market_odds",
        name: "Prediction Market Odds (via Polymarket)",
        description: "Get current odds and probability for a specific prediction market event. Returns data from Polymarket. Use when user asks 'what are the odds', 'event probability', 'prediction market odds', 'election betting odds', or 'market probability'.",
        inputSchema: {
          type: "object",
          properties: {
            slug: { type: "string", title: "Event Slug", description: "e.g., 'will-trump-win-2024'" },
        ...
        outputSchema: {
          type: "object",
          properties: {
            event: { type: "object", title: "Event Details" },
            odds: { type: "array", title: "Outcome Probabilities" }
          }
        },
        pricing: { amount: SERVICE_PRICING_USD["polymarket-odds"], currency: "USD" },
        category: "prediction_markets"
      },
      {
        id: "prediction_market_search",
        name: "Prediction Market Search (via Polymarket)",
        description: "Search prediction markets by keyword. Returns results from Polymarket with relevance scoring. Use when user asks 'search prediction markets', 'find markets about', 'prediction market for', or 'betting markets on topic'.",
        inputSchema: {
          type: "object",
          properties: {
            query: { type: "string", title: "Search Query", description: "e.g., 'bitcoin', 'election'" },
            limit: { type: "number", title: "Max Results" },
            exhaustive: { type: "boolean", title: "Search All Markets" },
            includeArchived: { type: "boolean", title: "Include Closed Markets" }
          },
          required: ["query"]
        },
        outputSchema: {
          type: "object",
          properties: {
            results: { type: "array", title: "Matching Markets" },
            count: { type: "number", title: "Results Found" },
            coverage: { type: "object", title: "Search Statistics" }
          }
        },
        pricing: { amount: SERVICE_PRICING_USD["polymarket-search"], currency: "USD" },
        category: "prediction_markets"
      }
    ],
