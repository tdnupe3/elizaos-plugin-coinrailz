...
export class A2ARegistryAdapter extends BaseDiscoveryAdapter {
  ...
  private async discoverFromRegistry(registry: any): Promise<DiscoveredAgentRaw[]> {
    ...
    try {
      ...
      switch (registry.name) {
        case 'AI16Z Registry':
          return await this.discoverAI16ZAgents(registry);
        case 'Virtuals Protocol Registry':
          return await this.discoverVirtualsAgents(registry);
        case 'AgentKit Registry':
          return await this.discoverAgentKitAgents(registry);
        case 'Google AP2 Directory':
          return await this.discoverGoogleAP2Agents(registry);
        case 'Farcaster Agent Directory':
          return await this.discoverFarcasterAgents(registry);
        case 'Base Agent Registry':
          return await this.discoverBaseAgents(registry);
        default:
  ...
  private async discoverAI16ZAgents(registry: any): Promise<DiscoveredAgentRaw[]> {
    const agents: DiscoveredAgentRaw[] = [];
    ...
    try {
      ...
      for (const agent of data.agents || []) {
        const normalizedAgent = this.normalizeAgent(agent, 'ai16z-registry');
        if (normalizedAgent) {
          // Add AI16Z specific metadata
