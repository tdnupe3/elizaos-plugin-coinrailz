...
type Step = "idle" | "switching" | "deploying" | "waiting" | "saving" | "done" | "error";
...
export default function VaultDeploy() {
  ...
  async function deploy() {
    ...
    try {
      ...
      // 3 — get fully-encoded deployment data from server
      setStep("deploying");
      ...
      if (!receipt) throw new Error("Tx sent but receipt not found after 3 minutes. Check Basescan for your tx hash above.");
  ...
  const isWorking = ["switching", "deploying", "waiting", "saving"].includes(step);
