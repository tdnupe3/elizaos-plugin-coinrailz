...
export interface ManagerSubscriptionOptions<TEvent> extends WsCadenceOptions {
  ...
  /** Return `undefined` to skip a notification (e.g. fail-soft on unknown discriminator). */
  decode: (address: Address, buffer: Uint8Array, slot: Slot) => TEvent | undefined;
...
export function createManagerSubscription<TEvent>(opts: ManagerSubscriptionOptions<TEvent>): SubscriptionHandle {
  ...
  return opts.manager.subscribeProgramAccounts(
    ...
    (address, buffer, slot) => {
      try {
        const event = opts.decode(address, buffer, slot);
        if (event !== undefined) opts.onChange(event);
      } catch (e) {
