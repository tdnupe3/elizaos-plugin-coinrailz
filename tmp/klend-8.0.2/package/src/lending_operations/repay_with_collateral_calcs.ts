 ...
 *   4. Withdraw `collWithdrawForFlashRepayLamports` (= flashBorrowIn * (1 + flashLoanFee)) of coll.
 ...
 * @throws if `swapPriceCollToDebt` is zero.
 */
export function calcRepayWithCollCollFlashSwap(props: {
  repayAmountLamports: Decimal;
  swapPriceCollToDebt: Decimal;
  slippagePct: Decimal;
  collFlashLoanFee: Decimal;
  collMintFactor: Decimal;
  ...
  const { repayAmountLamports, swapPriceCollToDebt, slippagePct, collFlashLoanFee, collMintFactor, debtMintFactor } =
    props;
  if (swapPriceCollToDebt.isZero()) {
    throw new Error('calcRepayWithCollCollFlashSwap: swapPriceCollToDebt is zero');
  }
  const slippageMultiplier = new Decimal(1).add(slippagePct.div('100'));
  // Convert debt repay amount to coll, padded for slippage so the swap output covers the repay.
  const debtToColl = repayAmountLamports.div(debtMintFactor).div(swapPriceCollToDebt).mul(collMintFactor);
  const collSwapInLamports = debtToColl.mul(slippageMultiplier).ceil();
  const flashBorrowInCollLamports = collSwapInLamports;
  const collWithdrawForFlashRepayLamports = flashBorrowInCollLamports.mul(new Decimal(1).add(collFlashLoanFee)).ceil();
  return {
