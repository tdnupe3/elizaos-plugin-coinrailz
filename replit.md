...
- **klic.gg eSports Partner API (Apr 1 2026) — BUILT & LIVE**: `server/routes/esportsPartnerRoutes.ts`. Three flows: (1) `POST /api/partner/esports/payout` — send USDC prize to winner via CDP; (2) `POST /api/partner/esports/session` — create payment session for entry fee or tip, returns platform wallet address + instructions; (3) `GET /api/partner/esports/transaction/:sessionId` — auto-detects on-chain USDC payment by scanning Transfer events from `payerWallet` to platform wallet (no txHash submission needed — attribution by sender address). Tips auto-forwarded to `streamerWallet` on confirmation. DB table: `esports_transactions`. Fee: 1.5%. Supported chains: Base, Ethereum. Public guide at `GET /api/partner/esports/guide`.
- **Stripe Connect: APPROVED (Mar 25 2026) — BUILD NEXT SESSION** — Kellogg Holdings LLC is approved. MAXIA (maxiaworld.app) is an active lead interested in Coin Railz as their payment processing layer for AI agent marketplace transactions. Build plan (in order): (1) Create `agent_payouts` DB table in `shared/schema.ts` + push migration; (2) Build connected account onboarding route — POST `/api/connect/accounts` creates Stripe connected account + returns Stripe-hosted onboarding redirect URL; (3) Wire `stripe.transfers.create()` into `releaseEscrowToAgent()` in `server/routes/aiMarketplaceRoutes.ts` (stub already there with TODO comment); (4) Add `account.updated` webhook handler in `server/routes/stripeRoutes.ts` to track when onboarding completes. Use Accounts v2 API (not v1 legacy). Platform fee: 15% via `application_fee_amount`. Stripe-hosted onboarding is fastest path — minimal code, Stripe handles KYC. After Connect is built, send MAXIA the integration email.
