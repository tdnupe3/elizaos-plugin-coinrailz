...
describe('GET /x402/vlt-usdc-withdraw — discovery', () => {
  test('returns service metadata without a payment header', async () => {
    ...
    expect(res.data.endpoint).toBe('POST /x402/vlt-usdc-withdraw');
    // Should expose schema / example
    expect(res.data.schema || res.data.example || res.data.network).toBeDefined();
  });
});
