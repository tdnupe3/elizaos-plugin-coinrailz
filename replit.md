...
- **x402SCAN VERSION STRING FIX (Dec 18, 2025)**: x402scan registration failed with "x402Version: Invalid input" error. Root cause: x402scan's Zod schema uses `z.enum(['1','2'])` which requires STRING type, but we were returning `x402Version: 2` (NUMBER). Fix: Changed all `x402Version: 2` to `x402Version: "2"` across paymentOrchestrator.ts, x402MicroserviceRoutesV2.ts, x402GatedRoutes.ts, and hybridPaymentMiddleware.ts. The x402ResponseEnricher also coerces to string as a safety net. This fix affects all 38+ x402 endpoints.
