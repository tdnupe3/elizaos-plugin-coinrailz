...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ==============================================
  // TRON BLOCKCHAIN INTEGRATION - USDT-TRC20 SUPPORT
  // Ultra-low fees for USDT transfers with fast settlement
  // ==============================================

  // Get Tron wallet information
  app.get("/api/tron/wallet/:address", async (req, res) => {
    try {
      const { address } = req.params;
      const walletInfo = await tronService.getWalletInfo(address);
      res.json({
        success: true,
        data: walletInfo,
        blockchain: 'tron',
        supportedTokens: ['TRX', 'USDT-TRC20']
      });
    } catch (error: any) {
  ...
  app.post("/api/tron/validate-address", async (req, res) => {
    try {
      const { address } = req.body;
      const isValid = await tronService.validateAddress(address);
      res.json({ valid: isValid, blockchain: 'tron' });
    } catch (error: any) {
  ...
  // Send TRX tokens
  app.post("/api/tron/send-trx", isAuthenticated, async (req, res) => {
    try {
      ...
      const user = await storage.getUser(userId);
      if (!user?.tronWallet) {
        return res.status(400).json({ error: "Tron wallet not configured" });
      }

      const txId = await tronService.sendTRX(user.tronWallet, toAddress, amount);
      ...
      await storage.createTransaction({
        ...
        metadata: { blockchain: 'tron', tokenType: 'TRX' }
      });
  ...
  app.post("/api/tron/send-usdt", isAuthenticated, async (req, res) => {
    try {
      const { toAddress, amount } = req.body;
      const userId = req.user?.claims?.sub;
      ...
      const txId = await tronService.sendUSDT(user.tronWallet, toAddress, amount);
      ...
      await storage.createTransaction({
        ...
        status: 'pending',
        externalTransactionId: txId,
        metadata: { blockchain: 'tron', tokenType: 'USDT-TRC20' }
      });
  ...
  app.get("/api/tron/transaction/:txId", async (req, res) => {
    try {
      ...
      console.error("Tron transaction status error:", error);
      res.status(500).json({ error: error.message });
    }
  });
  ...
  app.get("/api/tron/fees", async (req, res) => {
    try {
      ...
      const usdtFee = await tronService.estimateFee('USDT');
      const trxFeeUSD = await tronService.calculateFeeUSD('TRX');
      const usdtFeeUSD = await tronService.calculateFeeUSD('USDT');
