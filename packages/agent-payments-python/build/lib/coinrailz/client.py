...
class CoinRailz:
    ...
    def intelligence(
        ...
        Args:
            service: Service slug (e.g., 'wallet-risk', 'trade-signals', 'b20-token-info')
        ...
        if not re.match(r'^[a-z0-9-]{2,64}$', service):
            return ApiError(
                success=False,
                error="INVALID_SERVICE",
                message=f"Invalid service identifier '{service}'. See catalog: https://coinrailz.com/x402/catalog"
            )

        url = f"{self._base_url}/x402/{service}"
        headers = {
            "Authorization": f"Bearer {self._api_key}",
            "Content-Type": "application/json",
            "X-SDK-Version": SDK_VERSION,
            "User-Agent": f"coinrailz-python/{SDK_VERSION}",
        }

        try:
            response = self._client.post(url, headers=headers, json=params or {})
            ...
            if response.status_code == 402:
                ...
                return ApiError(
                    ...
                    message=f"Service '{service}' requires payment or credits. Top up at https://coinrailz.com/credits"
                )
            ...
            if response.status_code == 404:
                return ApiError(
                    ...
                    message=f"No x402 service found for '{service}'. See catalog: https://coinrailz.com/x402/catalog"
                )
            ...
            return IntelligenceServiceResult(
                success=True,
                service=service,
