...
- **GPT IN-CHAT CREDIT PURCHASE (Dec 18, 2025)**: Added 3 new endpoints for purchasing credits directly in ChatGPT: `/api/gpt/credits/packages`, `/api/gpt/credits/create-session`, `/api/gpt/credits/status`. Clean branded URLs: `https://coinrailz.com/pay/{sessionId}`. **DUAL PAYMENT MODE**: Feature flag `GPT_CHECKOUT_MODE=elements|checkout` controls payment flow. Elements mode embeds Stripe PaymentElement directly on coinrailz.com (stays on-domain, resists ad blockers). Checkout mode redirects to Stripe hosted checkout. Elements mode creates PaymentIntent with clientSecret stored in DB. Server route `/pay/:sessionId` checks session type and serves SPA for Elements or redirects for Checkout. Instant rollback by toggling env var. **⚠️ SCALING NOTE**: Uses in-memory `gptSessionCache` - requires Redis before multi-instance deployment.
- **HYBRID FACILITATOR FIX (Dec 18, 2025)**: Created shared `getFacilitatorUrl()` helper in `server/utils/facilitatorHelper.ts` that uses CDP facilitator (`https://facilitator.cdp.coinbase.com`) when CDP_API_KEY_ID is present, falls back to x402.org for testing. Updated all x402 files to use this helper. Root cause: Original hybrid logic was lost during V2 migration to hardcoded x402.org URL, causing Bazaar de-listing since Bazaar only indexes CDP facilitator services. Also registered MCP service discovery router in index.ts (was never imported). All discovery endpoints now correctly advertise CDP facilitator URL.
- **x402VERSION SPEC COMPLIANCE (Dec 18, 2025)**: x402scan registration showed "x402Version: Invalid input" warning because their Zod schema uses `z.enum(['1','2'])` expecting strings. However, after architect review of official Coinbase x402 spec (`interface PaymentRequiredResponse { x402Version: number }`), decision was made to KEEP NUMBER format for spec compliance with Coinbase Bazaar, CDP facilitator, and official SDKs. x402scan is non-compliant with official spec but still registered the resource despite the warning. All files use `x402Version: 2` (number) per official spec.
