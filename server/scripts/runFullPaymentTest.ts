...
// All 34 x402 services with test payloads
const ALL_SERVICES = [
  ...
  { name: "verified-agent-identity", endpoint: "/x402/verified-agent-identity", priceUSD: 5.00, payload: { agentName: "TestAgent", agentUrl: "https://test.com" } },
  { name: "compliance-consultation", endpoint: "/x402/compliance-consultation", priceUSD: 5.00, payload: { jurisdiction: "US", businessType: "crypto-exchange" } },
  { name: "smart-contract-audit", endpoint: "/x402/smart-contract-audit", priceUSD: 10.00, payload: { contractAddress: "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", chain: "ethereum" } },
...
async function runFullTest() {
  ...
  const totalCost = ALL_SERVICES.reduce((sum, s) => sum + s.priceUSD, 0);
  ...
  console.log("✅ Address matches! Starting full test suite...\n");
  ...
  for (let i = 0; i < ALL_SERVICES.length; i++) {
    ...
    console.log(`[${i + 1}/${ALL_SERVICES.length}] ${service.name} ($${service.priceUSD})...`);
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        failed++;
        failures.push(`${service.name}: ${response.status}`);
      }
    } catch (error: any) {
  ...
  console.log("═══════════════════════════════════════════════════════════════");
  console.log(`  Passed: ${passed}/${ALL_SERVICES.length}`);
  console.log(`  Failed: ${failed}/${ALL_SERVICES.length}`);
